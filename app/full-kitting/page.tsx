...
import { Loader2, AlertTriangle, History, CheckCircle, Settings, Plus, X } from "lucide-react"
...
export default function CheckPage() {
  ...
  const processGvizTable = (table: any) => {
    ...
    return table.rows.map((row: any, index: number) => {
        ...
        (row.c || []).forEach((cell: any, cellIndex: number) => {
            rowData[`col${cellIndex}`] = cell?.v ?? null;
        });
        return rowData;
    });
  };
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null)
    try {
      const [productionTable, kycTable] = await Promise.all([
      ...
      const pending = allRows.filter((row: any) => row.col20 != null && row.col21 == null)
        .map((row: any): ProductionItem => ({
          _rowIndex: row._rowIndex,
          jobCardNo: String(row.col1 || ''),
          deliveryOrderNo: String(row.col2 || ''),
          productName: String(row.col5 || ''),
          producedQuantity: Number(row.col20 || 0),
          verificationTimestamp: '',
          remarks: '',
        }));

      const history = allRows.filter((row: any) => row.col20 != null && row.col21 != null)
        .map((row: any): ProductionItem => ({
          _rowIndex: row._rowIndex,
          jobCardNo: String(row.col1 || ''),
          deliveryOrderNo: String(row.col2 || ''),
          productName: String(row.col5 || ''),
          producedQuantity: Number(row.col20 || 0),
          verificationTimestamp: String(row.col21 || ''),
          remarks: String(row.col65 || ''),
        })).sort((a, b) => {
          const dateA = parseGvizDate(a.verificationTimestamp);
          const dateB = parseGvizDate(b.verificationTimestamp);
          if (dateA && dateB) return dateB.getTime() - dateA.getTime();
          return 0;
        });
      
      const products = processGvizTable(kycTable).map((row: any): KycProduct => ({
  ...
  const resetKittingForm = () => {
      setKittingFormRows([{ id: 1, productName: '', percentage: '', baseAlumina: 0, baseIron: 0, basePrice: 0, baseBd: 0, baseAp: 0, al: 0, fe: 0, bd: 0, ap: 0, price: 0 }]);
  ...
  const addKittingFormRow = () => {
    if (kittingFormRows.length < 20) {
      setKittingFormRows([...kittingFormRows, { 
          id: (kittingFormRows[kittingFormRows.length-1]?.id || 0) + 1, productName: '', percentage: '', baseAlumina: 0, baseIron: 0, 
          basePrice: 0, baseBd: 0, baseAp: 0, al: 0, fe: 0, bd: 0, ap: 0, price: 0 
      }]);
    }
  };
  ...
  const handleKittingRowChange = (id: number, field: keyof KittingFormRow, value: any) => {
    const newRows = kittingFormRows.map(row => {
      if (row.id === id) {
        ...
        const percentage = parseFloat(updatedRow.percentage) || 0;
    ...
    const kittingTotals = useMemo(() => {
        return kittingFormRows.reduce((acc, row) => {
            acc.al += row.al;
            acc.fe += row.fe;
            acc.bd += row.bd;
            acc.ap += row.ap;
            acc.price += row.price;
            acc.percentage += parseFloat(row.percentage) || 0;
            return acc;
        }, { al: 0, fe: 0, bd: 0, ap: 0, price: 0, percentage: 0 });
    }, [kittingFormRows]);

    const interestAmount = useMemo(() => {
        return ((kittingTotals.price * 0.18 * (parseFloat(interestDays) || 0)) / 365);
    }, [kittingTotals.price, interestDays]);

    const totalCost = useMemo(() => {
        return kittingTotals.price + (parseFloat(manufacturingCost) || 0) + interestAmount + (parseFloat(transporting) || 0);
    }, [kittingTotals.price, manufacturingCost, interestAmount, transporting]);
    
    const sellingPrice = useMemo(() => {
        if (sellingPriceInput) return parseFloat(sellingPriceInput);
  ...
  const generateCompositionNumber = async (): Promise<string> => {
    ...
    let maxNumber = 0;
    rows.forEach(row => {
        const cn = row.col1; // Composition Number is in Column B (index 1)
        if (cn && typeof cn === 'string' && cn.startsWith('CN-')) {
            const num = parseInt(cn.substring(3));
  ...
  const handleSaveKittingForm = async () => {
    ...
    try {
        ...
        // Prepare Raw Material Names and Quantities (padded to 20)
        ...
        const rowData = [
            format(submissionDate, "dd/MM/yyyy HH:mm:ss"), // A: Timestamp
            compositionNumber,                             // B: Composition Number
            selectedCheck.deliveryOrderNo,                 // C: Order Number
            selectedCheck.productName,                     // D: Product Name
            variableCost.toFixed(2),                       // E: Variable Cost
            manufacturingCost,                             // F: Manufacturing Cost
            interestDays,                                  // G: Interest (days)
            interestAmount.toFixed(2),                     // H: Interest Cost
            transporting,                                  // I: Transporting Cost
            sellingPrice.toFixed(2),                       // J: Selling Price
            gpPercentage.toFixed(2) + "%",                 // K: GP %AGE
            kittingTotals.al.toFixed(4),                   // L: Alumina
            kittingTotals.fe.toFixed(4),                   // M: Iron
            kittingTotals.bd.toFixed(4),                   // N: BD
            kittingTotals.ap.toFixed(4),                   // O: AP
            ...paddedRmNames,                              // P-AI: Raw Material Names
            ...paddedRmQtys,                               // AJ-BC: Raw Material Qtys
        ];
  ...
  const handleSelectAllColumns = (tab: 'pending' | 'history', columnsMeta: any[], checked: boolean) => {
      const newVisibility = columnsMeta.reduce((acc, col) => {
  ...
  const visiblePendingColumnsMeta = useMemo(() => PENDING_COLUMNS_META.filter(c => visiblePendingColumns[c.dataKey]), [visiblePendingColumns]);
  const visibleHistoryColumnsMeta = useMemo(() => HISTORY_COLUMNS_META.filter(c => visibleHistoryColumns[c.dataKey]), [visibleHistoryColumns]);


  // --- Render Logic ---
  if (loading) return <div className="flex justify-center items-center h-screen"><Loader2 className="h-12 w-12 animate-spin text-purple-600" /></div>
  if (error) return <div className="p-8 text-center text-red-600"><AlertTriangle className="h-12 w-12 mx-auto" /><p>{error}</p></div>

  return (
    <div className="min-h-screen bg-white p-8">
      <Card>
        <CardHeader><CardTitle>Full Kitting Verification</CardTitle><CardDescription>Verify items after the full kitting process.</CardDescription></CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList>
              <TabsTrigger value="pending">Pending ({pendingChecks.length})</TabsTrigger>
              <TabsTrigger value="history">History ({historyChecks.length})</TabsTrigger>
            </TabsList>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex justify-between items-center">
                            ...
                            <Popover>
                                <PopoverTrigger asChild><Button variant="outline" size="sm"><Settings className="mr-2 h-4 w-4" />View</Button></PopoverTrigger>
                                <PopoverContent align="end" className="w-56">
                                    <div className="space-y-2 mb-4"><h4 className="font-medium leading-none">Toggle Columns</h4></div>
                                    <div className="flex justify-between">
                                        <Button variant="link" size="sm" className="p-0 h-auto" onClick={() => handleSelectAllColumns('pending', PENDING_COLUMNS_META, true)}>Select All</Button>
                                        <Button variant="link" size="sm" className="p-0 h-auto" onClick={() => handleSelectAllColumns('pending', PENDING_COLUMNS_META, false)}>Deselect All</Button>
                                    </div>
                                    <hr className="my-2" />
                                    {PENDING_COLUMNS_META.filter(c => c.toggleable).map(col => (
                                        <div key={col.dataKey} className="flex items-center space-x-2 my-1">
                                            <Checkbox id={`toggle-pending-${col.dataKey}`} checked={visiblePendingColumns[col.dataKey]} onCheckedChange={(checked) => handleToggleColumn('pending', col.dataKey, !!checked)} />
                                            <Label htmlFor={`toggle-pending-${col.dataKey}`} className="font-normal">{col.header}</Label>
                                        </div>
                                    ))}
                                </PopoverContent>
                            </Popover>
                    ...
                    <CardContent>
                        <Table>
                            <TableHeader><TableRow>{visiblePendingColumnsMeta.map(c => <TableHead key={c.dataKey}>{c.header}</TableHead>)}</TableRow></TableHeader>
                            <TableBody>
                            {pendingChecks.length > 0 ? pendingChecks.map(item => (
                                <TableRow key={item.jobCardNo}>
                                {visiblePendingColumnsMeta.map(col => (
                                    <TableCell key={col.dataKey}>
                                    {col.dataKey === 'actionColumn' ? (
                                        <Button size="sm" onClick={() => handleOpenKittingForm(item)}><CheckCircle className="mr-2 h-4 w-4" /> Verify</Button>
                                    ) : (item[col.dataKey as keyof ProductionItem] as React.ReactNode)}
                                    </TableCell>
                                ))}
                                </TableRow>
                            )) : <TableRow><TableCell colSpan={visiblePendingColumnsMeta.length} className="h-24 text-center">No pending items.</TableCell></TableRow>}
                            </TableBody>
                        </Table>
                    </CardContent>
                </Card>
            ...
            <TabsContent value="history">
              <Card>
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <Popover>
                            ...
                            <PopoverContent align="end" className="w-56">
                                <div className="space-y-2 mb-4"><h4 className="font-medium leading-none">Toggle Columns</h4></div>
                                <div className="flex justify-between">
                                    <Button variant="link" size="sm" className="p-0 h-auto" onClick={() => handleSelectAllColumns('history', HISTORY_COLUMNS_META, true)}>Select All</Button>
                                    <Button variant="link" size="sm" className="p-0 h-auto" onClick={() => handleSelectAllColumns('history', HISTORY_COLUMNS_META, false)}>Deselect All</Button>
                                </div>
                                <hr className="my-2" />
                                {HISTORY_COLUMNS_META.filter(c => c.toggleable).map(col => (
                                    <div key={col.dataKey} className="flex items-center space-x-2 my-1">
                                        <Checkbox id={`toggle-history-${col.dataKey}`} checked={visibleHistoryColumns[col.dataKey]} onCheckedChange={(checked) => handleToggleColumn('history', col.dataKey, !!checked)} />
                                        <Label htmlFor={`toggle-history-${col.dataKey}`} className="font-normal">{col.header}</Label>
                                    </div>
                                ))}
                            </PopoverContent>
                        </Popover>
                ...
                <CardContent>
                  <Table>
                    <TableHeader><TableRow>{visibleHistoryColumnsMeta.map(c => <TableHead key={c.dataKey}>{c.header}</TableHead>)}</TableRow></TableHeader>
                    <TableBody>
                      {historyChecks.length > 0 ? historyChecks.map(item => (
                        <TableRow key={item.jobCardNo}>
                          {visibleHistoryColumnsMeta.map(col => (<TableCell key={col.dataKey}>{item[col.dataKey as keyof ProductionItem] as React.ReactNode}</TableCell>))}
                        </TableRow>
                      )) : <TableRow><TableCell colSpan={visibleHistoryColumnsMeta.length} className="h-24 text-center">No history items.</TableCell></TableRow>}
                    </TableBody>
                  </Table>
                </CardContent>
              </Card>
        ...
        <DialogContent className="max-w-7xl w-full">
            ...
            <div className="max-h-[75vh] overflow-y-auto p-1">
                <div className="grid grid-cols-2 gap-4 mb-4 px-4">
                    <div>
                        <Label htmlFor="heatNumber">Job Card Number</Label>
                        <Input id="heatNumber" value={selectedCheck?.jobCardNo || ''} readOnly />
                    </div>
                    <div>
                ...
                <div className="flex justify-end mb-2 px-4">
                    <Button onClick={addKittingFormRow} disabled={kittingFormRows.length >= 20}><Plus className="h-4 w-4 mr-2" /> Add Row</Button>
                </div>
                        ...
                        <TableBody>
                            ...
                            <TableRow key={row.id}>
                                ...
                                <TableCell className="p-2">
                                    <Select onValueChange={(value) => handleKittingRowChange(row.id, 'productName', value)} value={row.productName}>
                                        <SelectTrigger><SelectValue placeholder="Select mat" /></SelectTrigger>
                                        <SelectContent>{kycProducts.map(p => <SelectItem key={p.productName} value={p.productName}>{p.productName}</SelectItem>)}</SelectContent>
                                    </Select>
                                </TableCell>
                                ...
                                <TableCell className="bg-yellow-100 p-2"><Input type="number" value={row.percentage} onChange={(e) => handleKittingRowChange(row.id, 'percentage', e.target.value)} placeholder="Enter %" /></TableCell>
                                ...
                                <TableCell className="p-2"><Button variant="ghost" size="icon" onClick={() => removeKittingFormRow(row.id)}><X className="h-4 w-4 text-red-500" /></Button></TableCell>
                            </TableRow>
                            ))}
                        </TableBody>
                        <TableFooter>
                            <TableRow>
                                <TableCell colSpan={7} className="text-right font-medium p-2">Manufacturing Cost</TableCell>
                                <TableCell colSpan={3} className="p-2"></TableCell>
                                <TableCell className="p-2"><Input type="number" value={manufacturingCost} onChange={(e) => setManufacturingCost(e.target.value)} /></TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell colSpan={7} className="text-right font-medium p-2">interest (days)</TableCell>
                                <TableCell className="bg-yellow-100 p-2"><Input type="number" value={interestDays} onChange={(e) => setInterestDays(e.target.value)} /></TableCell>
                                <TableCell colSpan={2} className="p-2"></TableCell>
                                <TableCell className="p-2"><Input value={interestAmount.toFixed(2)} readOnly /></TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell colSpan={7} className="text-right font-medium p-2">transporting (FOR)</TableCell>
                                <TableCell colSpan={4} className="p-2"><Input type="number" value={transporting} onChange={(e) => setTransporting(e.target.value)} /></TableCell>
                            </TableRow>
                            <TableRow className="font-bold">
                                <TableCell colSpan={7} className="text-right p-2">Total</TableCell>
                            ...
                            <TableRow>
                                <TableCell colSpan={7} className="text-right font-medium p-2">SELLING PRICE</TableCell>
                                <TableCell colSpan={4} className="p-2"><Input value={sellingPriceInput} onChange={e => setSellingPriceInput(e.target.value)} placeholder={sellingPrice.toFixed(2)} /></TableCell>
                            </TableRow>
                             <TableRow>
                                <TableCell colSpan={7} className="text-right font-medium p-2">VARIABLE COST</TableCell>
                                <TableCell colSpan={4} className="p-2"><Input value={variableCost.toFixed(2)} readOnly /></TableCell>
                            </TableRow>
                             <TableRow>
                                <TableCell colSpan={7} className="text-right font-medium p-2">GP %AGE</TableCell>
                                <TableCell colSpan={4} className="p-2"><Input value={gpPercentage.toFixed(2)+'%'} readOnly /></TableCell>
                            </TableRow>
                        </TableFooter>
                    </Table>
                </div>
            ...
            <div className="flex justify-end gap-2 p-4 border-t">
                ...
                <Button onClick={handleSaveKittingForm} disabled={isSubmitting}>
